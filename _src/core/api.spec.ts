...
test('onStart fires before fetch with correct context', async () => {
  ...
  const api = new CartApi({
    onStart: async (ctx) => { order.push('start'); startCtx = ctx; },
  });
  await api.add({ id: 1 });
...
test('onEnd fires when fetch fails', async () => {
  ...
  const api = new CartApi({ onEnd: async (ctx) => { endCtx = ctx; } });
  await api.add({ id: 1 });
...
test('async onStart blocks fetch until resolved', async () => {
  ...
  const api = new CartApi({
    onStart: () => new Promise<void>((resolve) => {
      order.push('start');
...
test('async onEnd blocks the returned promise until resolved', async () => {
  ...
  fetchMock.mockResolvedValue(mockResponse());
  const api = new CartApi({
    onEnd: () => new Promise<void>((resolve) => {
      endStarted = true;
      releaseEnd = () => { endFinished = true; resolve(); };
    }),
  });
  const promise = api.add({ id: 1 });
...
test('options.meta propagates to both hooks', async () => {
  ...
  fetchMock.mockResolvedValue(mockResponse());
  const api = new CartApi({
    onStart: async (ctx) => { startMeta = ctx.meta; },
    onEnd: async (ctx) => { endMeta = ctx.meta; },
  });
...
test('missing meta defaults to {} in hook context', async () => {
  let startMeta: unknown;
  fetchMock.mockResolvedValue(mockResponse());
  const api = new CartApi({ onStart: async (ctx) => { startMeta = ctx.meta; } });
  await api.add({ id: 1 });
...
test('subscriber abort() in onStart → fetch skipped, result is failure', async () => {
  fetchMock.mockResolvedValue(mockResponse());
  const api = new CartApi({
    onStart: async (ctx) => { ctx.abort('subscriber'); },
  });
...
test('onEnd still fires after subscriber-triggered abort', async () => {
  ...
  fetchMock.mockResolvedValue(mockResponse());
  const api = new CartApi({
    onStart: async (ctx) => { ctx.abort('cancel'); },
    onEnd: async (ctx) => { endCalled = true; endResult = ctx.result; },
  });
  await api.add({ id: 1 });
...
test.each(['add', 'change', 'update', 'clear', 'get'] as const)(
  ...
  async (method) => {
    ...
    fetchMock.mockResolvedValue(mockResponse());
    const api = new CartApi({ onStart: async (ctx) => { endpoint = ctx.endpoint; } });
